...
describe('Lifecycle', () => {
  it('The timer will work abnormally when the child component is toggled', async () => {
    const fatherWrapper = mount(ParentComp)
    const childWrapper = fatherWrapper.findComponent(ChildComp)
    ...
    await button.trigger('click')
    expect(fatherWrapper.findComponent(ChildComp).exists()).toBeTruthy()
